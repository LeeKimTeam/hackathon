...
class NextViewController: UIViewController {
  ...
  private func aler() {
    let alertController = UIAlertController(
      title: "결과값", message: "rrrrr", preferredStyle: .alert)
    let btnOk = UIAlertAction(
    title: "Okay", style: .cancel) /*, handle: nil) */ {
      (btnOk) in print("cencle")
    }
...
extension NextViewController: MKMapViewDelegate {
  ...
  func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
    ...
    annotationView.image = UIImage(named: "cctv2")
    annotationView.canShowCallout = true
    annotationView.calloutOffset = CGPoint(x: 0, y: 3)
